...
class NotebookPageController {
    ...
    async buildTrainmentLogTable(){
        const html = await fetchFile("mlp_log.html")
    ...
    sendMessage(){
        ...
        sendMessage({ values: Array.of(message) }, async (response) => {
            ...
            const possible_letters = formatted_data.filter((data) => data.formatted === 1)
            ...
            switch (possible_letters.length) {
                case 1:
                    ...
                    break
                case 2:
                    const predictedLetters = possible_letters.map( (data) => letters[data.index]).join(", ")
    ...
    attachEventListeners(){
        this.modal.querySelector("button").addEventListener("click", () => this.modal.close())
